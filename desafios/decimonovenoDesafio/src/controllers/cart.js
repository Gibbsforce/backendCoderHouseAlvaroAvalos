...
const addProductToCart = async (req, res) => {
    try {
        ...
        if (updated === undefined) return res.status(404).json({ message: "Cart not found" })
        if (updated === null) return res.status(400).json({
            message: "Error",
            error: "The cart is complete"
        })
        if (updated === false) return res.status(400).json({
            message: "Error",
            error: "Out of stock"
        })
...
const deleteProductFromCart = async (req, res) => {
    try {
        ...
        if (updated === undefined) return res.status(404).json({ message: "Cart not found" })
        if (updated === null) return res.status(404).json({ message: "Product not found" })
        if (updated === false) return res.status(400).json({ message: "Cart is empty" })
        res.status(200).json({
            message: "OK",
            cart: updated
        })
