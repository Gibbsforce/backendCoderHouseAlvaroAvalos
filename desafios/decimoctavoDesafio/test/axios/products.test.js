...
describe("TEST SUIT for products with axios", () => {
    ...
    describe("POST /api/products", () => {
        it("Should create a new product, return the id as an integer and return a status 201", async () => {
            ...
            assert.deepEqual(Number.isInteger(response.data.created) || typeof response.data.created === "string", true)
    ...
    describe("DELETE /api/products/:id", () => {
        it("Should delete a product by its id and return a status 200", async () => {
            ...
            assert.deepEqual(response.status, 200)
            assert.deepEqual(Number(response.data.id) || String(response.data.id), PRODUCT_ID)
        })
    })
})