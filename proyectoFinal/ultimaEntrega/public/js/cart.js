...
const carritosApi = {
  ...
  deleteCart: async (idCarrito) => {
    return await api.fetchDeleteCart(idCarrito)
  },
}

const orderAPI = {
  createOrder: async (order) => {
    return await api.fetchCreateOrder(order)
  },
  getOrder: async (id) => {
    return await api.fetchGetOrder(id)
  },
}
...
document.getElementById("btnCrearCarrito").addEventListener("click", () => {
  carritosApi.crearCarrito().then(({ cart }) => {
...
document
  ...
  .addEventListener("click", async () => {
    ...
    if (message === "Unauthorized")
      return alert("Please, login to complete the purchase")
    ...
    if (message === "OK") {
      ...
      const { message, error } = await orderAPI.createOrder(orderGenerated)
      if (message === "Error") return alert(error)
      if (message === "OK") {
        const { message, error } = await carritosApi.deleteCart(idCarrito)
        if (message === "Error") return alert(error)
        if (message === "OK") {
          alert(`Your order has been generated, an email has been sent`)
      ...
      html += `
                    ...
                    <td>
                        <div class="delete-btn">
                            <button calss="delete-btn-prod-from-cart" id="delete-${
                              prod._id
...
async function loadComboProductos() {
  return await productosApi.get().then((productos) => {
...
async function loadComboCarrito() {
  return await carritosApi.getIds().then(({ cart }) => {
    const combo = document.getElementById("comboCarritos")
    vaciarCombo(combo)
