...
const apiSettings = {
  ...
  fetchProductByCategory: async (category) => {
    const endpoint = `${PRODUCTS_URL}/category/${category}`
    if (graphQL === true) {
      const config = {
        ...defaultConfigPost,
        body: JSON.stringify({
          query: `{ getProductByCategory(category: ${JSON.stringify(
            category
          )}) { message error products { _id title description code thumbnail price stock timestamp } } }`,
        }),
      }
      return await (
        await (
  ...
  fetchCreateOrder: async (order) => {
    const endpoint = ORDERS_URL
    const config = {
      ...defaultConfigPost,
