...
class MongoContainer {
  ...
  update = async (id, obj) => {
    try {
      const { n, nModified } = await this.collection.updateOne(
        { _id: id },
        { $set: obj }
      )
  ...
  addToCart = async (id, product) => {
    try {
      ...
      const productExist = getCart.products.find(
        ({ _id }) => _id === product._id
      )
      if (productExist) {
  ...
  createUser = async (username, newUser) => {
    try {
      const user = await this.collection.find({ username: username })
      if (user[0]) return { user: true, message: "User already exists" }
      const createUser = await this.collection.create(newUser)
  ...
  getByCategory = async (category) => {
    try {
      const products = await this.collection.find({ category: category })
      if (products.length === 0) return null
      return products
    } catch (error) {
      logger.error(`Error at getting products by category: ${error}`)
    }
  }
  createOrder = async (order) => {
    try {
      const createOrder = await this.collection.create(order)
      if (!createOrder) return null
      return createOrder._id
    } catch (error) {
      logger.error(`Error at creating order: ${error}`)
    }
  }
  getOrders = async (userId) => {
    try {
      const orders = await this.collection.find({ userId: userId })
