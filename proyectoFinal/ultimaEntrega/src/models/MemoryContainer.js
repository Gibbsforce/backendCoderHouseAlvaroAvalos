...
class MemoryContainer {
  ...
  deleteFromCart = async (id, productId) => {
    try {
      ...
      const index = getCart.products.findIndex(
        ({ _id }) => parseInt(_id) === parseInt(productId)
      )
  ...
  createOrder = async (order) => {
    try {
      ...
      const orderId = await this.save(order)
  ...
  getOrders = async (userId) => {
    try {
      ...
      const userOrders = orders.filter(({ userId: id }) => id === userId)
      return userOrders
    } catch (error) {
